                ...
                return (
                  ...
                  >
                    <div className="flex justify-between items-start mb-4">
                      ...
                      <Link href={`/projects/${project.id}`} className="flex items-center justify-center w-8 h-8 text-gray-400 hover:text-white hover:bg-blue-600 rounded-full transition-all">
                        <ArrowRight className="w-5 h-5" />
                      </Link>
                    </div>

                    {/* Progress Bars - Updated Aesthetics */}
                    <div className="grid grid-cols-2 gap-6 mb-5 px-1">
                      <div>
                        <div className="flex justify-between text-xs mb-2 text-gray-500 uppercase tracking-wider font-semibold">
                    ...
                    <div className="flex items-center gap-2 pt-4 border-t border-gray-100 flex-wrap">
                      <Link href={`/gantt/${project.id}`} className="group flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-gray-600 bg-gray-50 hover:bg-blue-50 hover:text-blue-700 rounded-md transition-all border border-gray-200 hover:border-blue-200">
                        <GanttChartSquare className="w-4 h-4 text-gray-400 group-hover:text-blue-500" /> Gantt
                      </Link>
                      <Link href={`/scurve/${project.id}`} className="group flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-gray-600 bg-gray-50 hover:bg-indigo-50 hover:text-indigo-700 rounded-md transition-all border border-gray-200 hover:border-indigo-200">
                        <BarChart3 className="w-4 h-4 text-gray-400 group-hover:text-indigo-500" /> S-Curve
                      </Link>
                      <Link href={`/gantt-4w/${project.id}`} className="group flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-gray-600 bg-gray-50 hover:bg-purple-50 hover:text-purple-700 rounded-md transition-all border border-gray-200 hover:border-purple-200">
                        <CalendarDays className="w-4 h-4 text-gray-400 group-hover:text-purple-500" /> 4-Week
                      </Link>
                      <Link href={`/cost-code/${project.id}`} className="group flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-gray-600 bg-gray-50 hover:bg-emerald-50 hover:text-emerald-700 rounded-md transition-all border border-gray-200 hover:border-emerald-200">
                        <Target className="w-4 h-4 text-gray-400 group-hover:text-emerald-500" /> Cost
                      </Link>
                      <Link href={`/procurement/${project.id}`} className="group flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-gray-600 bg-gray-50 hover:bg-amber-50 hover:text-amber-700 rounded-md transition-all border border-gray-200 hover:border-amber-200">
            ...
            <div className="p-4">
              {criticalTasks.length === 0 ? (
                ...
                <div className="space-y-3">
                  {criticalTasks.map(task => {
                    ...
                    return (
                      <div key={task.id} className="group p-3 bg-white rounded-md border border-red-100 hover:border-red-300 hover:shadow-sm transition-all relative">
                        <div className="absolute left-0 top-0 bottom-0 w-1 bg-red-500 rounded-l-md"></div>
                        <div className="pl-2">
                          <div className="flex justify-between items-start mb-1.5">
                            <span className="text-[10px] font-bold text-red-600 bg-red-50 px-2 py-0.5 rounded-full uppercase tracking-wide border border-red-100">
                              {task.delayLabel}
                            </span>
              ...
              {criticalTasks.length > 0 && (
                <div className="mt-4 pt-3 text-center border-t border-gray-100">
                  <Link href="#" className="text-xs font-semibold text-red-600 hover:text-red-700 hover:underline">View all critical items</Link>
                </div>
              )}
            </div>
          </div>
