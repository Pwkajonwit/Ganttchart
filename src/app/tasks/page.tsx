...
export default function TasksPage() {
    ...
    const filteredTasks = useMemo(() => {
        ...
        // 5. Structure Hierarchy (Tree) only if sorting by default "order"
        // If sorting by other fields, flat list is arguably better
        if (sortBy === 'order') {
            // Define recursive type
            type EnhancedTask = Task & { children: EnhancedTask[]; level: number };
            ...
            // Initialize map
            result.forEach(t => {
                // Cast to EnhancedTask (children is initially empty)
                taskMap.set(t.id, { ...t, children: [], level: 0 } as EnhancedTask);
            });

            const rootTasks: EnhancedTask[] = [];
            ...
            const flatten = (nodes: EnhancedTask[], level = 0): EnhancedTask[] => {
                let flat: EnhancedTask[] = [];
                // Sort nodes by order
                ...
                nodes.forEach(node => {
                    ...
                    if (node.children.length > 0) {
                        flat = flat.concat(flatten(node.children, level + 1));
                    }
                });
                return flat;
            };
                                ...
                                {paginatedTasks.map((task: any) => (
                                    ...
                                    >
                                        <td className="px-4 py-3">
                                            <div className="max-w-[400px]" style={{ paddingLeft: `${(task.level || 0) * 24}px` }}>
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <p className={`text-sm truncate ${task.type === 'group' ? 'font-bold text-gray-900' : 'font-medium text-gray-800'}`}>
                                                        {task.name}
                                                    </p>
                                                </div>
                            ...
                            <form id="task-form" onSubmit={handleSubmit} className="space-y-4">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                    ...
                                    <div className="md:col-span-1">
                                        <label className="block text-sm font-medium text-gray-700 mb-1.5">หมวดหมู่ (ระดับ 1) *</label>
                                        <input
                                            type="text"
                                    ...
                                    {/* Subcategory */}
                                    <div className="md:col-span-1">
                                        <label className="block text-sm font-medium text-gray-700 mb-1.5">หมวดหมู่ย่อย (ระดับ 2)</label>
                                        <input
                                            type="text"
                                    ...
                                    {/* Name - Spans 1 col if subcat exists, else adjust */}
                                    <div className="md:col-span-2">
                                        <label className="block text-sm font-medium text-gray-700 mb-1.5">
                                            {taskForm.type === 'group' ? 'ชื่อกลุ่ม (ระดับ 4)' : 'ชื่องาน'}
                                        </label>
                                        <input
                                            type="text"
                                            required
